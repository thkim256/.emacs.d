...
(setq treemacs-read-string-input 'from-minibuffer; 입력창을 이맥스 기본 미니버퍼에서 실행
      treemacs-collapse-dirs (if treemacs-python-executable 3 0); 지정하지 않으면 간헐적 오류 발생
      ;; treemacs-width 35
...
#+RESULTS:
| (lambda nil (let ((lsp-keymap-prefix C-c l)) (lsp-enable-which-key-integration)) (setq gc-cons-threshold 100000000 lsp-idle-delay 0.2 lsp-ui-doc-enable nil lsp-log-io nil lsp-enable-symbol-highlighting nil read-process-output-max (* 1024 1024)) (push [/\\][^/\\]*\.\(json\ | html\ | yaml\)$ lsp-file-watch-ignored)) | lsp-lens-mode | dap-tooltip-update-mouse-motions-if-enabled | #[0 \305\306 \210)\307\310\311\312\211\207 [lsp-keymap-prefix gc-cons-threshold lsp-idle-delay lsp-ui-doc-enable read-process-output-max C-c l lsp-enable-which-key-integration 100000000 0.2 nil 1048576] 2] |
